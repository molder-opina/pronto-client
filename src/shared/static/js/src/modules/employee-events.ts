...
class EmployeeEventsManager {
  ...
  initialize(): void {
    ...
    document.addEventListener('employee:waiter-calls:updated', (event: Event) => {
      ...
      const normalized = calls
        .map((call) => this.normalizeRequest(call))
        .filter(Boolean) as NormalizedRequest[];
  ...
  private async loadClientRequests(): Promise<void> {
    ...
    try {
      ...
      const normalized = calls
        .map((call: WaiterCallPayload) => this.normalizeRequest(call))
        .filter(Boolean) as NormalizedRequest[];
      this.renderClientRequests(normalized);
    } catch (error) {
  ...
  private renderClientRequests(requests: NormalizedRequest[]): void {
    ...
    this.requestsContainer.innerHTML = requests
      .map((request) => {
        ...
        return `
                    <div class="request-card ${isUrgent ? 'request-card--urgent' : ''}" data-request-id="${
                      request.id
                    }">
      ...
      .join('');
    this.requestsContainer.querySelectorAll('[data-confirm-request]').forEach((btn) =>
  ...
  private async handleNewOrderNotification(data: any): Promise<void> {
    ...
    if (order_id) {
      try {
        ...
        if (!response.ok) {
          console.log(
            `[EmployeeEvents] Order #${order_id} no longer exists, skipping notification`
          );
          return;
        }
      } catch {
        ...
        console.log(
          `[EmployeeEvents] Could not verify order #${order_id}, proceeding with notification`
        );
      }
    }
    ...
    this.showDesktopNotification(
      'Nuevo Pedido',
      `Mesa ${table_number || 'desconocida'} - Pedido #${order_id}`,
      'new_order'
    );
  ...
  private handleWaiterCallNotification(data: WaiterCallPayload): void {
    ...
    const isCheckout =
      notes.includes('cuenta') || notes.includes('pagar') || notes === 'checkout_request';
    const message = isCheckout
      ? `La mesa ${data.table_number} solicitó la cuenta`
      : `La mesa ${data.table_number} solicita atención`;
    this.showDesktopNotification(
      isCheckout ? 'Solicitud de cuenta' : 'Llamada de mesa',
      message,
      'waiter_call'
    );
  ...
  private showDesktopNotification(title: string, body: string, tag: string): void {
    // Browser notifications disabled as requested.
    // We only use stylized UI notifications (toasts/modals).
    /*
