...
const ACTIONS: Partial<
  Record<LegacyWorkflowStatus, { label: string; endpoint: (id: number) => string }[]>
> = {
  requested: [
    { label: 'Iniciar preparación', endpoint: (id) => `/api/orders/${id}/kitchen/start` },
  ],
  waiter_accepted: [{ label: 'Iniciar', endpoint: (id) => `/api/orders/${id}/kitchen/start` }],
  kitchen_in_progress: [
    { label: 'Listo para entregar', endpoint: (id) => `/api/orders/${id}/kitchen/ready` },
  ],
  ready_for_delivery: [{ label: 'Entregado', endpoint: (id) => `/api/orders/${id}/deliver` }],
};

const STATUS_HINTS: Partial<Record<LegacyWorkflowStatus, string>> = {
...
class KitchenBoard {
  ...
  private initializeFilters(): void {
    ...
    this.workflowStatusFilters.add('requested');
  ...
  private getDisplayStatus(order: OrderInfo): string {
  ...
  private renderRowActions(row: HTMLTableRowElement, order: OrderInfo): void {
    ...
    if (statusEl) {
      statusEl.textContent = this.getDisplayStatus(normalizedOrder);
    ...
    const actions = this.canAdvanceKitchen ? ACTIONS[normalizedStatus] || [] : [];
    if (actions.length === 0) {
      const label = document.createElement('span');
      label.className = 'status-text';
      label.textContent = STATUS_HINTS[normalizedStatus] || this.getDisplayStatus(normalizedOrder);
  ...
  private createOrderRow(order: OrderInfo): HTMLTableRowElement {
    ...
    const row = document.createElement('tr');
    row.dataset.orderId = String(normalizedOrder.id);
    row.dataset.status = normalizedStatus;
    row.dataset.sessionId = String(normalizedOrder.session_id);
    row.dataset.sessionStatus = normalizedOrder.session?.status || '';
    row.dataset.paidAt = normalizedOrder.session?.closed_at || '';
    row.dataset.createdAt = normalizedOrder.created_at || '';
    row.dataset.sessionOpened = normalizedOrder.session?.opened_at || '';
    row.dataset.tableNumber = normalizedOrder.session?.table_number || '';
    row.dataset.customerName = normalizedOrder.customer?.name || '';
    row.dataset.customerEmail = normalizedOrder.customer?.email || '';
    row.dataset.customerId = String(normalizedOrder.customer?.id || '');
    row.dataset.notes = normalizedOrder.session?.notes || '';
    row.dataset.chefId = String(normalizedOrder.chef_id || '');
    row.dataset.waiterId = String(normalizedOrder.waiter_id || '');
    row.dataset.waiterName = normalizedOrder.waiter_name || '';
    ...
    const clientNotesHtml = normalizedOrder.session?.notes
      ? `
            <div class="kitchen-customer-note" style="margin-top: 0.5rem; padding: 0.5rem; background-color: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 4px;">
                ...
                <p style="margin: 0.25rem 0 0; font-size: 0.9rem; color: #78350f;">${escapeHtml(normalizedOrder.session.notes)}</p>
            </div>
        `
      : '';

    const orderId = escapeHtml(String(normalizedOrder.id));
    const tableNumber = escapeHtml(normalizedOrder.session?.table_number || '');
    const customerName = escapeHtml(normalizedOrder.customer?.name || 'Cliente');
    const customerEmail = escapeHtml(normalizedOrder.customer?.email || '');
    const customerPhone = escapeHtml(normalizedOrder.customer?.phone || '');
    const notes = escapeHtml(normalizedOrder.session?.notes || '');
    const workflowStatus = escapeHtml(normalizedOrder.workflow_status);
    const totalValue = escapeHtml(String(normalizedOrder.total_amount || 0));
  ...
  private initializePolling(): void {
    ...
    const pollForUpdates = async () => {
      try {
        ...
        const data = await response.json();
        const newOrders: OrderInfo[] = data.orders || [];
        ...
        if (addedOrders.length > 0) {
          ...
          showFeedback(
            this.feedback,
            `🆕 ${addedOrders.length} nueva(s) orden(es) - Mesa(s): ${tableNumbers}`
          );
