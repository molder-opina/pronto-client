...
class CashierBoard {
  ...
  private createOrderRow(order: OrderInfo): HTMLTableRowElement {
    ...
    row.dataset.customerEmail = normalizeCustomerEmail(normalizedOrder.customer?.email);
    row.dataset.customerId = normalizedOrder.customer?.id
      ? String(normalizedOrder.customer.id)
      : '';
    row.dataset.waiterName = normalizedOrder.waiter_name || '';
    row.dataset.waiterId = normalizedOrder.waiter_id ? String(normalizedOrder.waiter_id) : '';
    row.dataset.notes = normalizedOrder.session?.notes || '';
    row.dataset.total = normalizedOrder.total_amount ? String(normalizedOrder.total_amount) : '';
    ...
    const waiterBadge = normalizedOrder.waiter_name
      ? `<span class="waiter-badge">👤 ${escapeHtml(normalizedOrder.waiter_name)}</span>`
      : '<span class="waiter-badge waiter-badge--unassigned">⚠️ Sin asignar</span>';
    const totalDisplay = escapeHtml(`$${Number(normalizedOrder.total_amount || 0).toFixed(2)}`);
    const sessionStatus = escapeHtml(normalizedOrder.session?.status || '');
    const notesHtml = normalizedOrder.session?.notes
      ? `
