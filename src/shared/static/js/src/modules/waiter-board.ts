...
class WaiterBoard {
  ...
  private getDisplayStatus(order: WaiterOrder, sessionStatus?: string): string {
    if (sessionStatus === 'paid') return 'Pagado';
    if (order.status_display) return order.status_display;
    const normalized = normalizeWorkflowStatus(order.workflow_status, order.workflow_status_legacy);
    const map: Partial<Record<WorkflowStatus, string>> = {
      requested: 'Pendiente',
      waiter_accepted: 'Pendiente',
    ...
    return map[normalized] || formatStatus(normalized);
  }
  ...
  initialize(): void {
    ...
    // }
    this.pendingCallsInterval = window.setInterval(() => this.loadPendingCalls(), 30000);
  ...
  private renderExistingRows(): void {
    ...
    rows?.forEach((row) => {
      ...
      if (order) {
        ...
        order.workflow_status = normalized;
        order.workflow_status_legacy = normalized;
  ...
  private async processKitchenAction(button: HTMLButtonElement): Promise<void> {
    ...
    // Set up timeout to recover button if WebSocket doesn't respond
    const recoveryTimeout = window.setTimeout(() => {
      console.warn('[WAITER] WebSocket update timeout, recovering button state');
      button.disabled = false;
      button.className = originalClasses;
      button.replaceChildren(...originalNodes.map((n) => n.cloneNode(true)));
      window.showToast?.(
        'Acción procesada pero la actualización tardó. Recarga la página si no ves cambios.',
        'warning'
      );
    ...
    try {
      ...
      // OPTIMISTIC UPDATE: Immediately update button UI
      button.className = 'btn btn--small btn--success';
      button.replaceChildren(createFragment('✓ Procesado'));

      // Show success feedback
      notifyAction(this.feedbackEl, 'Acción de cocina procesada exitosamente');
      window.showToast?.('✓ Acción procesada exitosamente', 'success');

      // Clear recovery timeout since we succeeded
      window.clearTimeout(recoveryTimeout);

      // The socket update will handle the full UI refresh via order_status_changed event
      // But if it doesn't arrive within 5 seconds, user already saw success feedback
    } catch (error) {
      // Clear recovery timeout on error
      window.clearTimeout(recoveryTimeout);
  ...
  private async processWorkflowAction(button: HTMLButtonElement): Promise<void> {
    ...
    try {
      ...
      this.orders.set(orderId, normalizedData); // Update the order in the Map
      if (normalizedStatus === 'cancelled') {
        ...
        if (statusEl) {
          const displayStatus = this.getDisplayStatus(
            normalizedData,
            normalizedData.session?.status || row.dataset.sessionStatus
          );
          statusEl.textContent = displayStatus;
          statusEl.className = `status status--${normalizedStatus}`;
        }
        row.dataset.waiterNotes = normalizedData.waiter_notes || '';
        row.dataset.waiterName = normalizedData.waiter_name || row.dataset.waiterName || '';
        row.dataset.sessionStatus = normalizedData.session?.status || '';
        row.dataset.tableNumber = normalizedData.session?.table_number || '';
        this.renderRow(row, normalizedData);
      }
      ...
      const tableNumber =
        normalizedData.session?.table_number || row.dataset.tableNumber || '#' + orderId;

      if (normalizedStatus === 'waiter_accepted') {
      ...
      } else if (normalizedStatus === 'kitchen_in_progress') {
        notificationMessage = `🍳 Orden #${orderId} enviada a cocina`;
      }

      if (normalizedData.session) {
        this.updateSharedSessionState(normalizedData.session);
      }
      document.dispatchEvent(new CustomEvent('orders:changed'));
  ...
  private renderRow(row: HTMLTableRowElement, order: WaiterOrder): void {
    ...
    const actionLabelText =
      this.getDisplayStatus(order, sessionStatus) || statusInfo?.label || 'Acciones';
    ...
    if (allowedActions.length === 0 && order.workflow_status === 'waiter_accepted') {
      allowedActions = [
        {
          label: 'En Cocina',
          icon: '🍳',
    ...
    if (allowedActions.length === 0 && order.workflow_status === 'requested') {
      allowedActions = [
        {
          label: 'Aceptar',
          icon: '✅',
    ...
    if (statusEl) {
      ...
      const isOverdue =
        ...
        ['kitchen_in_progress', 'ready_for_delivery'].includes(order.workflow_status) &&
    ...
    const shouldShowPaymentActions =
      canProcessPayments && sessionStatus !== 'paid' && order.workflow_status === 'delivered';
    ...
    if (shouldShowPaymentActions) {
      ...
      this.renderPaymentActions(actionsContainer, order);
      return;
    }
    ...
    if (statusInfo && allowedActions.length > 0) {
      ...
      if (order.workflow_status === 'ready_for_delivery' && order.items && order.items.length > 1) {
      ...
      this.socket.on('order_status_changed', (data: any) => {
        ...
        if (data?.order_id && this.orders.has(data.order_id)) {
          ...
          if (existingOrder && data.workflow_status) {
            ...
            existingOrder.workflow_status = normalized;
            existingOrder.workflow_status_legacy = normalized;
            if (data.status_display) {
              existingOrder.status_display = data.status_display;
            }
            const row = this.ordersTable?.querySelector<HTMLTableRowElement>(
        ...
        this.ordersTable
          ...
          .forEach((row) => {
            ...
            if (order) {
              order.session = { ...order.session, status: 'paid' };
              order.workflow_status = 'delivered';
              order.workflow_status_legacy = 'delivered';
              this.renderRow(row, order);
            }
          });
        void this.loadPaidSessions();
      ...
      this.socket.on('orders.auto_accepted', (data: any) => {
        if (data?.waiter_id === this.currentEmployeeId) {
          ...
          if (data.order_id && this.orders.has(data.order_id)) {
            ...
            const existingOrder = this.orders.get(data.order_id);
            if (existingOrder) {
              existingOrder.workflow_status = 'waiter_accepted';
              existingOrder.workflow_status_legacy = 'waiter_accepted';
              ...
              if (row) {
                row.dataset.status = 'waiter_accepted';
  ...
  private updateOrderStatus(orderId: number, status: WorkflowStatus): void {
    ...
    const normalized = normalizeWorkflowStatus(status, existingOrder.workflow_status_legacy);
    existingOrder.workflow_status = normalized;
    existingOrder.workflow_status_legacy = normalized;
    const row = this.ordersTable?.querySelector<HTMLTableRowElement>(
      `tr[data-order-id="${orderId}"]`
  ...
  private resetFilters(): void {
    ...
    this.workflowStatusFilters = new Set([
      'requested',
      'waiter_accepted',
      'kitchen_in_progress',
      'ready_for_delivery',
  ...
  private applyFilters(): void {
    ...
    rows.forEach((row) => {
      ...
      const isRequested = order.workflow_status === 'requested';
  ...
  public async refreshOrders(showLoading: boolean = false): Promise<void> {
    try {
      ...
      newOrders.forEach((order) => this.orders.set(order.id, order));
    ...
    try {
      ...
      if (newOrderIds.length > 0) {
        ...
        if (ordersResponse.ok) {
          ...
          const allOrders: WaiterOrder[] = (ordersData.orders || []).map((order: WaiterOrder) => {
            const normalized = normalizeWorkflowStatus(
              order.workflow_status,
              order.workflow_status_legacy
            );
  ...
  private initializeOrdersPolling(): void {
    ...
    // Capture start time to avoid notifying about old orders found on first poll
    ...
    const pollForUpdates = async () => {
      try {
        ...
        const addedOrders = newOrders
          ...
          // Solo notificar órdenes realmente nuevas (solicitadas) y con sesión activa
          // Y que hayan sido creadas DESPUÉS de que iniciamos el polling (evita flood en login)
          .filter((o) => {
            const sessionStatus = o.session?.status || '';
            const createdAtMs = o.created_at ? new Date(o.created_at).getTime() : 0;
            const isFresh = createdAtMs > pollingStartTime;

            return (
              o.workflow_status === 'requested' &&
    ...
    // Start polling
    this.ordersPollingInterval = window.setInterval(pollForUpdates, POLL_INTERVAL_MS);
  ...
  private async confirmPartialDelivery(): Promise<void> {
    ...
    try {
      ...
      const data = await response.json();
      const allDelivered = data.items?.every((item: any) => item.is_fully_delivered) || false;
      const normalizedStatus = normalizeWorkflowStatus(
        data.workflow_status,
        data.workflow_status_legacy
      );
      ...
      this.orders.set(this.partialDeliveryOrderId, normalizedData);

      // Close modal
      this.closePartialDeliveryModal();

      // If all items are delivered, automatically mark the order as delivered
      if (allDelivered && normalizedStatus === 'ready_for_delivery') {
        try {
          ...
          const deliverNormalized = normalizeWorkflowStatus(
            deliverResponse.workflow_status,
            deliverResponse.workflow_status_legacy
          );
          const deliverData = {
            ...deliverResponse,
            workflow_status: deliverNormalized,
            workflow_status_legacy: deliverNormalized,
          } as WaiterOrder;
          this.orders.set(this.partialDeliveryOrderId, deliverData);
          ...
          if (row) {
            this.renderRow(row, deliverResponse);
          }
        } catch (error) {
