"""
Waiter authentication routes using JWT.
"""
...
waiter_bp = Blueprint(
    "waiter_app", __name__, url_prefix="/waiter", template_folder="../../templates/waiter"
)
...
def logout():
    ...
    if user:
        try:
            ...
            current_app.logger.error(f"Error signing out waiter in DB: {e}")

    response = make_response(redirect(url_for("waiter_app.login")))
...
def login():
    user = get_current_user()
    if user and user.get("active_scope") == "waiter":
        return redirect(url_for("waiter_app.dashboard"))
...
def process_login():
    ...
    if not email or not password:
        return render_template("login_waiter.html", error="Credenciales requeridas")
    ...
    try:
        with get_session() as db_session:
            ...
            if not employee or not employee.verify_password(password):
                return render_template("login_waiter.html", error="Credenciales inválidas")

            if not employee.has_scope("waiter"):
                return render_template("login_waiter.html", error="No tiene permisos de Mesero")
            ...
            response = make_response(redirect(url_for("waiter_app.dashboard")))
            response.set_cookie(
                "access_token",
                access_token,
                httponly=True,
        ...
        current_app.logger.error(f"Waiter login error: {e}")
        return render_template("login_waiter.html", error="Error de sistema")


@waiter_bp.route("/dashboard")
def dashboard():
    """
    Waiter dashboard - shows active orders and waiter operations.
    """
    ...
    from shared.services.role_service import list_employees_by_permission

    user = get_current_user()
    employee_role = user.get("employee_role", "waiter") if user else "waiter"
    ...
    employees = list_employees_by_permission("waiter-board")
    ...
    currency_code = get_config_value("currency_code", "USD")
    currency_symbol = get_config_value("currency_symbol", "$")

    # Get role capabilities for waiter
    role_capabilities = {
        "can_accept_orders": True,
        "can_deliver_orders": True,
        "can_cancel_orders": True,
        "can_edit_orders": True,
        "can_move_tables": True,
        "can_process_payments": employee_role in ["admin", "cashier", "system"],
        "can_reprint_tickets": employee_role in ["admin", "cashier", "system"],
    }
    ...
    return render_template(
        ...
        can_process_payments=role_capabilities.get("can_process_payments", False),
...
@waiter_bp.route("/system_login", methods=["GET", "POST"])
@csrf.exempt
def system_login():
    """
    ...
    if request.method == "GET":
        return render_template("login_waiter.html", error="Método no permitido")

    raw_token = request.form.get("token")
    if not raw_token:
        return redirect(url_for("waiter_app.login"))
    ...
    try:
        with get_session() as db_session:
            ...
            result = db_session.execute(
                ...
                .where(
                    ...
                    SuperAdminHandoffToken.target_scope == "waiter",
            ...
            if result.rowcount != 1:
                db_session.rollback()
                current_app.logger.warning("Invalid handoff token attempt for waiter scope")
                return redirect(url_for("waiter_app.login"))
            ...
            if not employee or not employee.has_scope("waiter"):
                db_session.rollback()
                return redirect(url_for("waiter_app.login"))
            ...
            current_app.logger.info(
                f"System handoff successful: employee_id={employee.id}, scope=waiter"
            )
        ...
        current_app.logger.error(f"System login error (waiter): {e}")
        return redirect(url_for("waiter_app.login"))
