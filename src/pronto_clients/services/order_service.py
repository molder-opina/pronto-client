...
from pronto_shared.services.price_service import (
...
def _debug(msg: str, **kwargs):
    """Debug helper that logs to logger.debug"""
    if kwargs:
        msg = f"{msg} | {kwargs}"
    logger.debug(f"[DEBUG_ORDER] {msg}")
    ...
    if require_contact:
        ...
        if not has_email and not has_phone:
            raise OrderValidationError(
                "Proporciona tu email o teléfono para recibir el ticket"
            )
...
def validate_required_modifiers(
    ...
    menu_item_groups = session.execute(
        ...
        .join(
            ModifierGroup, MenuItemModifierGroup.modifier_group_id == ModifierGroup.id
        )
...
def create_order(
    ...
    with get_session() as session:
        ...
        _debug(
            "Starting customer creation/lookup", email=email_value, phone=phone_value
        )
        ...
        customer = (
            ...
            .filter(Customer.email_hash == email_hash)
            .one_or_none()
        )
        if customer is None:
            customer = Customer()
        ...
        def get_valid_table(
            recorded_table_number: str | None, allow_fallback: bool = False
        ):
            ...
            if allow_fallback:
                ...
                return (
                    session.execute(
                        select(Table).where(Table.is_active).order_by(Table.id)
                    )
                    .scalars()
                    .first()
                )
        ...
        resolved_table_number = (
            table_record.table_number if table_record else table_number
        )
        ...
        if existing_session_id:
            ...
            if dining_session and dining_session.status == "open":
                ...
                if getattr(dining_session, "is_expired", False):
                    logger.info(
                        f"Session {existing_session_id} has expired, closing it"
                    )
        ...
        if dining_session is None:
            ...
            if table_id:
                ...
                if existing_by_table and not getattr(
                    existing_by_table, "is_expired", False
                ):
            ...
            if table_id and dining_session.table_id != table_id:
                ...
                fallback_table = get_valid_table(
                    dining_session.table_number, allow_fallback=False
                )
                if fallback_table:
        ...
        order = Order(
            customer=customer, session=dining_session, customer_email=email_value
        )
        session.add(order)
        session.flush()
        ...
        total_display_accumulated = Decimal(
            "0"
        )  # Sum of display prices (what user sees)
        ...
        for i, item in enumerate(items_data):
            try:
                ...
                _debug(
                    f"Selected modifiers for item {menu_item.name}: {selected_modifiers}"
                )
                ...
                item_breakdown = calculate_price_breakdown(
                    line_price, tax_decimal, price_mode
                )
        ...
        if cached_table_id:
            from pronto_shared.services.waiter_table_assignment_service import (
            ...
            try:
                ...
                if assignment and assignment.get("waiter_id"):
                    ...
                    order_requires_kitchen = any(
                        not item.menu_item
                        or not getattr(item.menu_item, "is_quick_serve", False)
        ...
        if order.workflow_status == OrderStatus.CANCELLED.value:
            logger.warning(
                "New order %s created with cancelled status; resetting to new", order.id
            )
            order.mark_status(OrderStatus.NEW.value)
        ...
        order_id = (
            sqla_inspect(order).identity[0]
            if sqla_inspect(order).identity
            else order.id
        )
        session_id = (
